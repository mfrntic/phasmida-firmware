...
// Saved UI hooks (filled by the calling main.cpp)
AppUiHooks g_hooks;

// Boot log callback (set by headful main via g_hooks.onBootLogLine)
// logLine routes to this if set, so boot messages reach BootLogScreen.
std::function<void(const String&)> g_bootLogLineCb;
...
void logLine(const String& line) {
  Serial.println(line);
  if (g_hooks.onBootLogLine) {
    g_hooks.onBootLogLine(line);
  }
}
...
bool publishEvent(const char* type, const char* severity, const char* message,
                  const char* detailsJson = nullptr);
bool publishTelemetry();
bool publishStatus(const char* state, const char* reason = nullptr);
bool publishCommandAck(const String& cmdId, const char* status,
                       const char* errorCode = nullptr,
                       const char* errorMessage = nullptr,
                       const char* resultJson = nullptr);

// ── Probe presence callback ────────────────────────────────────────────────────
...
bool publishStatus(const char* state, const char* reason) {
...
// Probes are registered by main.cpp, so locate the soil probe by its
// telemetry type. `present` reports whether it is physically connected.
SoilMoistureProbe* findSoilProbe(bool* present = nullptr) {
  for (size_t i = 0; i < g_probes.probeCount(); ++i) {
    ISensorProbe* p = g_probes.probeAt(i);
    if (p && strcmp(p->telemetryType(), "soil-moisture") == 0) {
      if (present) *present = g_probes.isPresent(i);
      return static_cast<SoilMoistureProbe*>(p);
    }
  }
...
void applySoilCalibration(uint16_t dryMv, uint16_t wetMv, bool persist) {
  if (SoilMoistureProbe* soil = findSoilProbe()) {
    soil->setCalibration(dryMv, wetMv);
  }
...
void handleMqttCommand(const String& topicStr, const String& payloadStr) {
  ...
  if (type == "request-telemetry") {
    ...
    publishCommandAck(cmdId, ok ? "ok" : "error",
                      ok ? nullptr : "telemetry_failed",
                      ok ? nullptr : "Telemetry publish failed");
    return;
  }
  ...
  if (type == "set-config") {
    ...
    uint16_t soilDryMv = cmd["params"]["soilDryMv"] | g_runtimeCfg.soilDryMv;
    uint16_t soilWetMv = cmd["params"]["soilWetMv"] | g_runtimeCfg.soilWetMv;
    ...
    g_runtimeCfg.telemetryIntervalMs = g_telemetryIntervalMs;
    ...
    if (soilDryMv != g_runtimeCfg.soilDryMv || soilWetMv != g_runtimeCfg.soilWetMv) {
  ...
  if (type == "calibrate-soil") {
    ...
    if (point == "reset") {
      ...
      bool present = false;
      SoilMoistureProbe* soil = findSoilProbe(&present);
      if (!soil || !present) {
        publishCommandAck(cmdId, "rejected", "probe_not_present", "Soil probe is not connected");
        return;
      }
      uint16_t mv    = soil->readMilliVolts();
      uint16_t dryMv = (point == "dry") ? mv : soil->dryMv();
      uint16_t wetMv = (point == "wet") ? mv : soil->wetMv();
      if (!isValidSoilCalibration(dryMv, wetMv, AppConfig::kSoilMoistureMaxMv)) {
        char msg[96];
        snprintf(msg, sizeof(msg), "Captured %u mV would put the wet point at/above the dry point (%u/%u)",
                 static_cast<unsigned>(mv), static_cast<unsigned>(dryMv), static_cast<unsigned>(wetMv));
        publishCommandAck(cmdId, "rejected", "invalid_soil_calibration", msg);
        return;
      }
    ...
    snprintf(resultJson, sizeof(resultJson),
             ...
             point.c_str(),
             static_cast<unsigned>(g_runtimeCfg.soilDryMv),
             static_cast<unsigned>(g_runtimeCfg.soilWetMv));
    publishCommandAck(cmdId, "ok", nullptr, nullptr, resultJson);
    return;
  }
  ...
  if (type == "set-timezone") {
    ...
    char resultJson[128];
    snprintf(resultJson, sizeof(resultJson),
             "{\"appliedTz\":\"%s\",\"clockMode\":\"local\"}", posixTz.c_str());
    publishCommandAck(cmdId, "ok", nullptr, nullptr, resultJson);
    return;
  }
  ...
  if (type == "start-rgb-verification") {
    ...
    if (ackOk) {
      ...
      if (g_hooks.onRgbVerifyStart) {
        g_hooks.onRgbVerifyStart(sessionId, confirmWindowMs, millis());
      }
      char resultJson[192];
      snprintf(resultJson, sizeof(resultJson),
  ...
  if (type == "confirm-rgb-verification" || type == "reject-rgb-verification") {
    if (!g_rgbVerification.isPending()) {
      publishCommandAck(cmdId, "rejected", "no_pending_verification",
                        "No RGB verification session is currently pending");
      return;
    }
    String sessionId = cmd["params"]["sessionId"] | "";
    String activeSession = g_rgbVerification.activeSessionId();
    if (!sessionId.isEmpty() && sessionId != activeSession) {
      publishCommandAck(cmdId, "rejected", "session_id_mismatch",
                        "Provided sessionId does not match active pending session");
      return;
    }
    if (type == "confirm-rgb-verification") {
    ...
    char resultJson[192];
    snprintf(resultJson, sizeof(resultJson),
             "{\"sessionId\":\"%s\",\"state\":\"resolved\",\"action\":\"%s\"}",
             activeSession.c_str(),
             type == "confirm-rgb-verification" ? "confirm" : "reject");
    publishCommandAck(cmdId, "ok", nullptr, nullptr, resultJson);
    return;
  }
  ...
  if (type == "factory-reset") {
    ...
    if (!confirm) {
      publishCommandAck(cmdId, "rejected", "confirm_required",
...
static size_t publishProbe(ISensorProbe* probe, const SensorReading& r, uint64_t tsMs) {
  ...
  logf("Telemetry [%s]: %s (%u metric%s)",
       probe->name(), ok ? "ok" : "fail",
       static_cast<unsigned>(cnt), cnt == 1 ? "" : "s");
  return ok ? cnt : 0;
}
...
void appRegisterProbe(ISensorProbe* probe) {
  g_probes.addProbe(probe);
}

void appBegin(const AppUiHooks& hooks) {
  // --- HOOK: Provjera je li bila aktivna RGB verifikacija prije reboota ---
  // TODO: Ovdje bi trebalo učitati zadnju sessionId iz NVRAM/RTC/flash i ako je bila pending,
  // odmah publishRgbVerificationResult(..., "timeout", "confirm_timeout", ...)
  ...
  uint32_t bootStep = 0;
  bootLog(bootStep, "Device starting");
  bootLog(bootStep, "Initializing identity and MQTT topics");
  g_identity.init();
  bootLog(bootStep, "Identity and MQTT topics initialized");
  ...
  bootLog(bootStep, "Opening NVS namespace");
  if (!g_configStore.begin()) {
    logLine("NVS: failed to open preferences namespace");
  } else {
    bootLog(bootStep, "NVS namespace opened");
  }

  bootLog(bootStep, "Loading runtime config from NVS");
  g_configStore.loadDefaults();

  if (AppConfig::kEnableWifiProvisioning && !g_configStore.hasWifiCredentials()) {
    bootLog(bootStep, "No WiFi credentials in NVS; entering provisioning mode");
    String newSsid;
    String newPassword;
  ...
  // Probes are already registered (appRegisterProbe runs before appBegin).
  ...
  bootLog(bootStep, "Runtime configuration loaded");
  ...
  if (g_hooks.onUiSetupScreens) {
    g_hooks.onUiSetupScreens(g_probes);
  }

  bootLog(bootStep, "Detecting + initializing probes");
  g_probes.setSampleIntervalMs(g_telemetryIntervalMs);
  g_probes.begin();
  for (size_t i = 0; i < g_probes.probeCount(); ++i) {
  ...
  if (g_hooks.onUiPostProbeInit) {
    g_hooks.onUiPostProbeInit();
  }

  bootLog(bootStep, "Starting WiFi connection phase");
  ...
  if (wifiOk) {
    bootLog(bootStep, "Starting NTP sync phase");
    ...
    bootLog(bootStep, String("NTP sync phase completed: ") + (g_timeSync.isSynced() ? "SYNCED" : "NOT_SYNCED"));
  } else {
  ...
  bootLog(bootStep, "MQTT client configured");

  if (wifiOk) {
    bootLog(bootStep, "Starting MQTT connection phase");
    logLine("--- MQTT CONNECT PHASE ---");
    bool mqttOk = g_mqtt.connect();
    bootLog(bootStep, String("MQTT phase completed: ") + (mqttOk ? "SUCCESS" : "FAILED"));
  } else {
    bootLog(bootStep, "Skipping MQTT connect (WiFi disconnected)");
  }

  bootLog(bootStep, "Scheduling periodic tasks");
  g_nextTelemetryAt = millis() + g_telemetryIntervalMs;
  g_nextStatusHeartbeatAt = millis() + AppConfig::kStatusHeartbeatMs;
  bootLog(bootStep, "Periodic tasks scheduled");

  // RGB Verification: wire up result callback
  g_rgbVerification.begin(g_ledMgr,
    [](const String& sessionId, const char* result, const char* reason,
       uint32_t durationMs, uint32_t confirmWindowMs, const char* pattern) {
      if (g_hooks.onRgbVerifyEnd) {
        g_hooks.onRgbVerifyEnd();
      }
      publishRgbVerificationResult(sessionId, result, reason,
                                   durationMs, confirmWindowMs, pattern);
    });
  bootLog(bootStep, "RGB verification manager initialized");

  logLine("========== BOOT COMPLETE ==========");
  logLine("boot sequence completed");
  bootLog(bootStep, "Boot sequence completed");
  ...
  if (g_hooks.onBootComplete) {
    g_hooks.onBootComplete();
  }
}

LedManager& appGetLedManager() {
  return g_ledMgr;
}

ConfigStore& appGetConfigStore() {
  return g_configStore;
}
