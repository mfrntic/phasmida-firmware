...
void MqttClient::begin(const MqttConfig& cfg) {
  _cfg = cfg;
  _reconnectDelayMs = AppConfig::kMqttReconnectBaseDelayMs;
  _nextReconnectAt  = 0;
  _instance         = this;
...
bool MqttClient::connect() {
  ...
  // Keep preflight as plain TCP only; TLS handshake is handled by PubSubClient.
  WiFiClient tcpProbe;
...
void MqttClient::_scheduleReconnect(bool authFailure) {
  if (authFailure) {
    _reconnectDelayMs = max(_reconnectDelayMs, AppConfig::kMqttAuthFailureInitialDelayMs);
  }
  uint32_t nextDelay = _addJitter(_reconnectDelayMs);
  _nextReconnectAt   = millis() + nextDelay;
  _reconnectDelayMs  = min(_reconnectDelayMs * 2U, AppConfig::kMqttReconnectMaxDelayMs);
  if (_onStatusChange) {
    char buf[24];
